 ...
 *
 *  @brief Function to convert network host names and ports into C++ Networking TS
 *  endpoint objects.
 *
 *  @author Cliff Green
 *  @date 2017, 2018
...
/**
 *  @brief Create a set of endpoints and return them in a function object callback, performing
 *  name resolving (DNS lookup) as needed.
 *
...
void make_endpoint(std::experimental::net::io_context& ioc, F&& func, bool local,
  ...
  if (local) {
    resolver.async_resolve(host_or_intf_name, service_or_port, 
                   (ip::resolver_base::passive | ip::resolver_base::address_configured) , func);
  }
  else {
    resolver.async_resolve(host_or_intf_name, service_or_port, func);
  }
  return;
}

...
void net_entity_base_test(chops::const_shared_buffer buf, int num_bufs,
...
SCENARIO ( "Io base test, udp", "[io_base_udp]" ) {
  ...
  net_entity_base_test<udp_io_mock>(chops::const_shared_buffer(std::move(mb)), 20,
...
SCENARIO ( "Io base test, tcp", "[io_base_tcp]" ) {
  ...
  chops::mutable_shared_buffer mb(ba.data(), ba.size());
  net_entity_base_test<tcp_io_mock>(chops::const_shared_buffer(std::move(mb)), 40,
